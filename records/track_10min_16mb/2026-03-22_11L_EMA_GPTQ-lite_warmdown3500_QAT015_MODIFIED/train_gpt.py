...
class Hyperparameters:
    ...
    mlp_tie_factors = bool(int(os.environ.get("MLP_TIE_FACTORS", "0")))
    use_flash_attention = bool(int(os.environ.get("USE_FLASH_ATTENTION", "0")))
    q_rank = int(os.environ.get("Q_RANK", 0))
    q_tie_transpose = bool(int(os.environ.get("Q_TIE_TRANSPOSE", "0")))
    ...
    def step(self, closure=None):
        ...
        for group in self.param_groups:
            ...
            for p in params:
                ...
                curr += p.numel()
        return loss
def flash_attn_3_or_sdpa(q: Tensor, k: Tensor, v: Tensor, causal: bool, use_flash_attention: bool) -> Tensor:
    if use_flash_attention:
        if flash_attn_3_func is None:
            raise ImportError(
                "USE_FLASH_ATTENTION=1 was set, but flash_attn_interface is not installed."
            )
        return flash_attn_3_func(q, k, v, causal=causal)
    # SDPA expects [B, H, T, D], while this model uses [B, T, H, D].
    q_t = q.transpose(1, 2)
...
class CausalSelfAttention(nn.Module):
    def __init__(
        ...
        self.q_rank = q_rank if 0 < q_rank < dim else 0
        self.q_tie_transpose = bool(q_tie_transpose and self.q_rank > 0)
        self.use_flash_attention = use_flash_attention
        if self.q_rank > 0:
            self.c_q_down = CastedLinear(dim, self.q_rank, bias=False)
            self.c_q_up = None if self.q_tie_transpose else CastedLinear(self.q_rank, dim, bias=False)
            self.c_q = None
        else:
    ...
    def forward(self, x: Tensor, v_embed: Tensor | None = None) -> Tensor:
        ...
        if self.q_rank > 0:
            q_down = self.c_q_down(x)
            if self.q_tie_transpose:
                q_proj = F.linear(q_down, self.c_q_down.weight.T.to(q_down.dtype))
            else:
                q_proj = self.c_q_up(q_down)
        else:
            q_proj = self.c_q(x)
        ...
        y = flash_attn_3_or_sdpa(q, k, v, causal=True, use_flash_attention=self.use_flash_attention)
        if self.use_xsa:
...
class ValueEmbedding(nn.Module):
    ...
    def forward(self, token_ids: Tensor) -> Tensor:
        ...
        return h * self.scale.to(dtype=h.dtype)
class MLP(nn.Module):
    def __init__(self, dim: int, mlp_mult: int, mlp_rank: int = 0, tie_factors: bool = False):
        super().__init__()
        hidden = int(mlp_mult * dim)
        self.mlp_rank = mlp_rank if 0 < mlp_rank < hidden else 0
        self.tie_factors = bool(tie_factors and self.mlp_rank > 0)
        if self.mlp_rank > 0:
            self.fc_down = CastedLinear(dim, self.mlp_rank, bias=False)
            self.fc_up = CastedLinear(self.mlp_rank, hidden, bias=False)
            self.proj_down = None if self.tie_factors else CastedLinear(hidden, self.mlp_rank, bias=False)
            self.proj_up = None if self.tie_factors else CastedLinear(self.mlp_rank, dim, bias=False)
    ...
    def forward(self, x: Tensor) -> Tensor:
        if self.mlp_rank > 0:
            ...
            if self.tie_factors:
                x = F.linear(x, self.fc_up.weight.T.to(x.dtype))
                return F.linear(x, self.fc_down.weight.T.to(x.dtype))
            x = self.proj_down(x)
...
class Block(nn.Module):
    def __init__(
        ...
        self.attn = CausalSelfAttention(dim, num_heads, num_kv_heads, q_rank, q_tie_transpose, use_flash_attention, rope_base, qk_gain_init)
        self.mlp = MLP(dim, mlp_mult, mlp_rank=mlp_rank, tie_factors=mlp_tie_factors)
...
def eval_val_sliding(
    ...
    compiled_logits = (
        ...
        if args.use_torch_compile else base_model.forward_logits
    )
    with torch.inference_mode():
...
def main() -> None:
    ...
    base_model = GPT(
        ...
        mlp_tie_factors=args.mlp_tie_factors,
        q_rank=args.q_rank,
        q_tie_transpose=args.q_tie_transpose,
    ...
    if master_process:
        ...
        if args.save_models_by_run_id:
            ...
            run_model_path = f"models/{args.run_id}_final_model.pt"
            torch.save(export_sd, run_model_path)
    ...
    if master_process:
        ...
        if args.save_models_by_run_id:
            os.makedirs("models", exist_ok=True)
            run_quant_path = f"models/{args.run_id}_final_model.int6.ptz"
            with open(run_quant_path, "wb") as f:
                f.write(quant_blob)
    ...
    eval_model = GPT(
        ...
        mlp_rank=args.mlp_rank, mlp_tie_factors=args.mlp_tie_factors,
        q_rank=args.q_rank, q_tie_transpose=args.q_tie_transpose,
        use_flash_attention=args.use_flash_attention,
